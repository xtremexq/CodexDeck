...
Deck does not write failover banners or background switch notices over Codex's full-screen terminal. Codex supports a local shell prefix, so use `!account`, `!pool`, `!usage`, `!check`, and `!deck`; they execute locally without submitting anything to the model. Every signed-in ordinary conversation gets a manual-only loopback route, so `!account` can switch from `codex-auth account5` even while automatic failover is Off. It prints the numbered session-account list; named environments also show their current pool. `!pool` prints that pool directly. Because Codex local shell commands do not expose interactive stdin, switch with a second command such as `!account account7`, `!pool 2`, or `!pool account7`. `!check` reproduces the complete `codex-check` report. `!usage` first resolves the session's active route and checks only that account, so it follows manual switches. `!deck` opens the desktop companion. Codex does not expose a third-party bare-slash extension point, so these cannot appear as `/account` entries in its built-in menu. Earlier Deck-managed `$account`, `$pool`, `$check`, and `$usage` skills are removed during update because skills necessarily create a model turn. Local session history, settings and tools remain attached to the starting account/environment; only subsequent API requests change routes. Desktop Settings saves the default and pool; desktop terminal launches and the terminal dashboard use the same preference. Refreshing usage remains an explicit command or dashboard operation.

Run `!autocompact` in a managed interactive session to request compaction of its active conversation. It uses the selected Compaction implementation in Settings: Custom asks the agent for the configured handoff, compacts, and replays it; Native invokes Codex compaction after the active turn. The command works with automatic compaction switched off.
