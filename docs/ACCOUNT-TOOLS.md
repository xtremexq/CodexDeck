...
This is an experimental HTTP Responses mode. Requests containing account-scoped response references, file IDs, or encrypted history cannot safely switch accounts after quota rejection: the proxy stops with an explanation instead. This can prevent failover later in a conversation or after compaction. It does not reconstruct or silently discard conversation history; start a fresh session when that limit is reached. Full-context requests without those references can continue across accounts, including compaction requests. History produced by the active account remains usable after a switch; the upstream service validates opaque history. Unsupported endpoints and concurrent conversation requests are rejected explicitly; model-list requests can run alongside a conversation.
