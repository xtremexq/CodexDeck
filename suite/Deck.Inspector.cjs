...
q('#list').onclick=event=>{const target=event.target.closest('[data-action]');if(!target)return;const action=target.dataset.action,key=target.dataset.key;if(action==='expand'){target.closest('.item').classList.toggle('open');return}if(action==='edit'){const item=items.get(key),rule=ruleMap().get(key);editing=key;q('#editText').value=rule?.action==='edit'?rule.text:(item?.text||'');q('#editor').showModal();q('#editText').focus();return}act(action,key)};q('#search').oninput=render;q('#filter').onchange=render;q('#largest').onclick=()=>{largest=!largest;q('#largest').classList.toggle('on',largest);render()};q('#restore').onclick=()=>act('clear');q('#cancel').onclick=()=>q('#editor').close();q('#save').onclick=async()=>{const text=q('#editText').value;q('#editor').close();await act('edit',editing,text)};document.addEventListener('keydown',event=>{if(event.ctrlKey&&event.key.toLowerCase()==='f'){event.preventDefault();q('#search').focus();q('#search').select()}});if(!/^[a-f0-9]{24}$/.test(live||'')){q('#list').replaceChildren(node('div','empty','This companion link is invalid.'));q('#status').textContent='Unavailable'}else{refresh(true);setInterval(()=>refresh(false),700)}
</script></body></html>`}
