...
function textOf(value) {
  ...
  for (const field of ['text','output','arguments','input','content','summary']) {
    if (value[field] != null) { const content=textOf(value[field]); if (content) return content; }
  }
  return '';
}
...
function normalize(row, line) {
  ...
  if (row.type === 'session_meta') {
    ...
    if (!base.text && payload.type === 'reasoning' && payload.encrypted_content) base.text='Encrypted reasoning; plaintext is unavailable';
...
async function scanSession(session) {
  ...
  for await (const line of lines) {
    ...
    if (row.type === 'response_item' && payload.type && /call/.test(payload.type) && !/output/.test(payload.type)) {
      ...
      const input=textOf(payload.arguments ?? payload.input);
...
function renderSessionStatus(){
  ...
  q('#contextFree').title=capacity?'Approximate text/JSON size of request input, instructions and tools · '+fmt(estimated)+' / '+fmt(capacity)+' tokens · '+(session.model||'unknown model'):'Model context capacity unavailable';
